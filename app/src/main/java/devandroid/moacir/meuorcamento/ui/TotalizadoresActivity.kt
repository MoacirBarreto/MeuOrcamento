...
class TotalizadoresActivity : AppCompatActivity() {
    ...
    private fun atualizarUI(lancamentos: List<LancamentoComCategoria>) {
        // Chama a função de animação, passando o container do XML como alvo
        animarConteudo(binding.containerAnalise) {
            // ---- TODO O CÓDIGO ORIGINAL DE ATUALIZARUI VAI AQUI DENTRO ----
            ...
            if (saldo < BigDecimal.ZERO) {
                binding.tvPeriodoSaldo.setTextColor(ContextCompat.getColor(this, R.color.app_vermelho_despesa))
            } else {
    ...
    private fun atualizarGraficoDeBarras(totalReceitas: Float, totalDespesas: Float) {
        ...
        val receitaDataSet = BarDataSet(listOf(receitaEntry), "Receita").apply {
            color = ContextCompat.getColor(this@TotalizadoresActivity, R.color.app_verde_receita)
            valueTextColor = Color.BLACK
            valueTextSize = 12f
        }
        val despesaDataSet = BarDataSet(listOf(despesaEntry), "Despesa").apply {
            color = ContextCompat.getColor(this@TotalizadoresActivity, R.color.app_vermelho_despesa)
            valueTextColor = Color.BLACK
            valueTextSize = 12f
        }
